...
echo -e "${BLUE}P2P Port: ${GREEN}${P2P_PORT}${NC}"
echo -e "${BLUE}Pruning: ${GREEN}${PRUNING_MODE}${NC}"
echo ""
# --- System Preparation ---
...
# Set pruning based on user input
if [ "$PRUNING_MODE" == "custom" ]; then
    ...
    sed -i "s|^pruning *=.*|pruning = \"nothing\"|" $HOME/.empe-chain/config/app.toml
    echo -e "${GREEN}Pruning set to nothing${NC}"
else
    sed -i "s|^pruning *=.*|pruning = \"default\"|" $HOME/.empe-chain/config/app.toml
...
sed -i "s#laddr = \"tcp://0.0.0.0:26657\"#laddr = \"tcp://0.0.0.0:$RPC_PORT\"#" $HOME/.empe-chain/config/config.toml
sed -i "s#laddr = \"tcp://0.0.0.0:26656\"#laddr = \"tcp://0.0.0.0:$P2P_PORT\"#" $HOME/.empe-chain/config/config.toml
...
echo -e "Pruning: ${GREEN}${PRUNING_MODE}${NC}"
echo ""
